...
const FoodOrders = new Schema ({
    ...
    status:{type:String, enum:Object.values(FoodOrderStatusEnum), required:true},
    createdAt:{type:Date,default:Date.now, immutable:true},
    updatedAt:{type:Date,default:Date.now}, 
})
...
app.post("/addfoods", async(req: Request, res: Response)=>{
    ...
    try{
        await FoodsModel.create({foodName: food, price, image, ingredients, category: isCategoryExisted?._id})
        res.send("success")
    } catch(err){
...
app.post("/addFoodOrders", async(req: Request, res: Response)=>{
    const {user, totalPrice, foodOrderItems, status, createdAt, updatedAt }= req.body
    try{
        await FoodOrdersModel.create({user, totalPrice, foodOrderItems, status, createdAt, updatedAt })
        res.send("success")
    } catch(err){
        console.log(err);
